...
# Stop after 15 clicks
            if len(self.selected_points) >= self.max_clicks:
                ...
                # Step 2. Apply to all images using the saved conditions
                for file in files:
                    im = Image.open(file)
...
if __name__ == "__main__":
    ...
    files = sorted(image_dir.glob("*.*"))  # adjust extensions as needed
    
    # Step 1. Select on the first image only
    reference = files[0]
    selector = InteractiveSelector(reference)
